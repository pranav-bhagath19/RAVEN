...
class BanditContextVector(BaseModel):
    ...
    tenant_id: str = Field(default="tenant_default", description="Authenticated Tenant ID")
    payment_id: str = Field(default="pay_default", description="Target Payment ID")
    amount_scaled: float = Field(default=0.1, ge=0.0, description="Amount in INR scaled by 1,000,000")
    attempts_count: float = Field(default=1.0, ge=0.0, description="Previous failure attempts count")
    currency_code_encoded: float = Field(default=1.0, description="Encoded currency code (1.0 for INR)")
    error_code_encoded: float = Field(default=0.0, description="Encoded failure error code")
    root_cause_encoded: float = Field(default=0.0, description="Encoded failure root cause")
    action_type_encoded: float = Field(default=1.0, description="Encoded candidate action type")
    merchant_status_encoded: float = Field(default=1.0, description="Encoded merchant status")
    customer_opt_out_flag: float = Field(default=0.0, description="Customer opt-out flag (0.0 or 1.0)")
    systemic_downtime_flag: float = Field(default=0.0, description="Systemic downtime flag (0.0 or 1.0)")
    base_propensity: float = Field(default=0.5, ge=0.0, le=1.0, description="Base ML propensity score")
    tenant_action_success_rate: float = Field(default=0.5, ge=0.0, le=1.0, description="Tenant empirical action success rate")
    global_action_success_rate: float = Field(default=0.5, ge=0.0, le=1.0, description="Global empirical action success rate")
